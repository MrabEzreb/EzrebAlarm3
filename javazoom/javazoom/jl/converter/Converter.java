...
public class Converter {
	...
	public synchronized void convert(InputStream sourceStream, String destName,
		...
		if (progressListener == null)
			progressListener = PrintWriterProgressListener
		...
		try {
			...
			progressListener.converterUpdate(
					ProgressListener.UPDATE_FRAME_COUNT, frameCount, 0);
			...
			progressListener.converterUpdate(
					ProgressListener.UPDATE_CONVERT_COMPLETE, time, frame);
	...
	static public class PrintWriterProgressListener implements ProgressListener {
		...
		static public PrintWriterProgressListener newStdOut(int detail) {
			return new PrintWriterProgressListener(new PrintWriter(System.out,
					true), detail);
		}
		...
		public void converterUpdate(int updateID, int param1, int param2) {
			if (isDetail(VERBOSE_DETAIL)) {
				...
				case UPDATE_CONVERT_COMPLETE:
					...
					pw.println();
					pw.println("Converted " + param2 + " frames in " + param1
							+ " ms (" + (param1 / param2) + " ms per frame.)");
				}
			}
		}
