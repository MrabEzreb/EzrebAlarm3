...
public class WaveFile extends RiffFile {
	...
	class WaveFormat_ChunkData {
		...
		public void Config(int NewSamplingRate, short NewBitsPerSample,
		...
		public int VerifyValidity() {
			boolean ret = header.ckID == FourCC("fmt ")
					...
					data.nAvgBytesPerSec == (data.nChannels
							* data.nSamplesPerSec * data.nBitsPerSample) / 8
					&&
    ...
    *
	*/
	public int OpenForWrite(String Filename, int SamplingRate,
			short BitsPerSample, short NumChannels) {
		// Verify parameters...
		if ((Filename == null) || (BitsPerSample != 8 && BitsPerSample != 16)
		...
		if (retcode == DDC_SUCCESS) {
			...
			if (retcode == DDC_SUCCESS) {
				...
				/*
				 * byte[] br = new byte[16]; br[0] = (byte)
				 * ((wave_format.data.wFormatTag >> 8) & 0x00FF); br[1] = (byte)
				 * (wave_format.data.wFormatTag & 0x00FF);
				 * 
				 * br[2] = (byte) ((wave_format.data.nChannels >> 8) & 0x00FF);
				 * br[3] = (byte) (wave_format.data.nChannels & 0x00FF);
				 * 
				 * br[4] = (byte) ((wave_format.data.nSamplesPerSec >> 24)&
				 * 0x000000FF); br[5] = (byte) ((wave_format.data.nSamplesPerSec
				 * >> 16)& 0x000000FF); br[6] = (byte)
				 * ((wave_format.data.nSamplesPerSec >> 8)& 0x000000FF); br[7] =
				 * (byte) (wave_format.data.nSamplesPerSec & 0x000000FF);
				 * 
				 * br[8] = (byte) ((wave_format.data.nAvgBytesPerSec>> 24)&
				 * 0x000000FF); br[9] = (byte)
				 * ((wave_format.data.nAvgBytesPerSec >> 16)& 0x000000FF);
				 * br[10] = (byte) ((wave_format.data.nAvgBytesPerSec >> 8)&
				 * 0x000000FF); br[11] = (byte)
				 * (wave_format.data.nAvgBytesPerSec & 0x000000FF);
				 * 
				 * br[12] = (byte) ((wave_format.data.nBlockAlign >> 8) &
				 * 0x00FF); br[13] = (byte) (wave_format.data.nBlockAlign &
				 * 0x00FF);
				 * 
				 * br[14] = (byte) ((wave_format.data.nBitsPerSample >> 8) &
				 * 0x00FF); br[15] = (byte) (wave_format.data.nBitsPerSample &
				 * 0x00FF); retcode = Write (br, 16);
				 */
	...
	*/
	 ...
	 * ChunkName.getBytes(0,4,p,0); int ret = (((p[0] << 24)& 0xFF000000) |
	 * ((p[1] << 16)&0x00FF0000) | ((p[2] << 8)&0x0000FF00) |
	 * (p[3]&0x000000FF)); return ret; }
	 */
