...
public class WaveFileObuffer extends Obuffer {
	...
	public WaveFileObuffer(int number_of_channels, int freq, String FileName) {
		...
		int rc = outWave.OpenForWrite(FileName, freq, (short) 16,
				(short) channels);
	}
	...
	public void write_buffer(int val) {
		 ...
		 * (((buffer[j+1]>>8)&0x000000FF) | ((buffer[j+1]<<8)&0x0000FF00));
		 * myBuffer[0] = buffer[j]; myBuffer[1] = buffer[j+1]; rc =
		 * outWave.WriteData (myBuffer,2); }
		 */
   ...
   */
	 ...
	 * Obuffer thebuffer = null; int mode = maplay_args.MPEGheader.mode(); int
