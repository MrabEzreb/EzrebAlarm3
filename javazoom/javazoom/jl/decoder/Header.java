...
public final class Header {
	public static final int[][] frequencies = { { 22050, 24000, 16000, 1 },
			{ 44100, 48000, 32000, 1 }, { 11025, 12000, 8000, 1 } }; // SZD:
																		// MPEG25
	...
	private int h_layer, h_protection_bit, h_bitrate_index, h_padding_bit,
			h_mode_extension;
	...
	void read_header(Bitstream stream, Crc16[] crcp) throws BitstreamException {
			...
			if (syncmode == Bitstream.INITIAL_SYNC) {
				...
				if (((headerstring >>> 20) & 1) == 0) // SZD: MPEG2.5 detection
					if (h_version == MPEG2_LSF)
						...
						throw stream
								.newBitstreamException(BitstreamErrors.UNKNOWN_ERROR);
		...
		if (h_sample_frequency == FOURTYFOUR_POINT_ONE) {
			/*
			 * if (offset == null) { int max = max_number_of_frames(stream);
			 * offset = new int[max]; for(int i=0; i<max; i++) offset[i] = 0; }
			 * // E.B : Investigate more int cf = stream.current_frame(); int lf
	...
	void parseVBR(byte[] firstframe) throws BitstreamException {
		...
		try {
			...
			if (xing.equals(new String(tmp))) {
				...
				System.arraycopy(firstframe, offset + length, flags, 0,
						flags.length);
				length += flags.length;
				// Read number of frames (if available).
				if ((flags[3] & (byte) (1 << 0)) != 0) {
					System.arraycopy(firstframe, offset + length, tmp, 0,
							tmp.length);
					h_vbr_frames = (tmp[0] << 24) & 0xFF000000 | (tmp[1] << 16)
							& 0x00FF0000 | (tmp[2] << 8) & 0x0000FF00 | tmp[3]
							& 0x000000FF;
					length += 4;
				}
				...
				if ((flags[3] & (byte) (1 << 1)) != 0) {
					System.arraycopy(firstframe, offset + length, tmp, 0,
							tmp.length);
					h_vbr_bytes = (tmp[0] << 24) & 0xFF000000 | (tmp[1] << 16)
							& 0x00FF0000 | (tmp[2] << 8) & 0x0000FF00 | tmp[3]
							& 0x000000FF;
					length += 4;
				}
				...
				if ((flags[3] & (byte) (1 << 2)) != 0) {
					System.arraycopy(firstframe, offset + length, h_vbr_toc, 0,
							h_vbr_toc.length);
				...
				if ((flags[3] & (byte) (1 << 3)) != 0) {
					System.arraycopy(firstframe, offset + length, tmp, 0,
							tmp.length);
					h_vbr_scale = (tmp[0] << 24) & 0xFF000000 | (tmp[1] << 16)
							& 0x00FF0000 | (tmp[2] << 8) & 0x0000FF00 | tmp[3]
							& 0x000000FF;
					length += 4;
				}
	...
	public static final int bitrates[][][] = {
			...
			{
					{ 0 /* free format */, 32000, 64000, 96000, 128000, 160000,
