...
final class SynthesisFilter {
	 ...
	 * HIGH_QUALITY: case MEDIUM_QUALITY: case LOW_QUALITY: v_inc = 16 *
	 * quality0; quality = quality0; break; default : throw new
	...
	private void compute_new_v() {
		 ...
		 * new_v[16-31]) into other v: v1[16 + actual_write_pos] = new_v16;
		 * v1[32 + actual_write_pos] = new_v17; v1[48 + actual_write_pos] =
		 * new_v18; v1[64 + actual_write_pos] = new_v19; v1[80 +
		 * actual_write_pos] = new_v20; v1[96 + actual_write_pos] = new_v21;
		 * v1[112 + actual_write_pos] = new_v22; v1[128 + actual_write_pos] =
		 * new_v23; v1[144 + actual_write_pos] = new_v24; v1[160 +
		 * actual_write_pos] = new_v25; v1[176 + actual_write_pos] = new_v26;
		 * v1[192 + actual_write_pos] = new_v27; v1[208 + actual_write_pos] =
		 ...
		 * actual_write_pos] = new_v30; v1[288 + actual_write_pos] = new_v29;
		 * v1[304 + actual_write_pos] = new_v28; v1[320 + actual_write_pos] =
		 * new_v27; v1[336 + actual_write_pos] = new_v26; v1[352 +
		 * actual_write_pos] = new_v25; v1[368 + actual_write_pos] = new_v24;
		 * v1[384 + actual_write_pos] = new_v23; v1[400 + actual_write_pos] =
		 * new_v22; v1[416 + actual_write_pos] = new_v21; v1[432 +
		 * actual_write_pos] = new_v20; v1[448 + actual_write_pos] = new_v19;
		 * v1[464 + actual_write_pos] = new_v18; v1[480 + actual_write_pos] =
	...
	private void compute_new_v_old() {
		...
		// coding like this is hard to read
		float tmp2;
		new_v[5] = (new_v[11] = (new_v[13] = (new_v[15] = p[15]) + p[7])
				+ p[11])
				+ p[5] + p[13];
		new_v[7] = (new_v[9] = p[15] + p[11] + p[3]) + p[13];
		new_v[33 - 17] = -(new_v[1] = (tmp1 = p[13] + p[15] + p[9]) + p[1])
				- p[14];
		new_v[35 - 17] = -(new_v[3] = tmp1 + p[5] + p[7]) - p[6] - p[14];

		new_v[39 - 17] = (tmp1 = -p[10] - p[11] - p[14] - p[15]) - p[13] - p[2]
				- p[3];
	...
	private void compute_pcm_samples(Obuffer buffer) {
		 ...
		 * buffer.append(channel, (short)tmpOut[i]); } break; case 64: for (int
		 * i=0; i<8; i++) { buffer.append(channel, (short)tmpOut[i]);
	...
	private static final float cos1_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI / 64.0)));
	private static final float cos3_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 3.0 / 64.0)));
	private static final float cos5_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 5.0 / 64.0)));
	private static final float cos7_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 7.0 / 64.0)));
	private static final float cos9_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 9.0 / 64.0)));
	private static final float cos11_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 11.0 / 64.0)));
	private static final float cos13_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 13.0 / 64.0)));
	private static final float cos15_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 15.0 / 64.0)));
	private static final float cos17_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 17.0 / 64.0)));
	private static final float cos19_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 19.0 / 64.0)));
	private static final float cos21_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 21.0 / 64.0)));
	private static final float cos23_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 23.0 / 64.0)));
	private static final float cos25_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 25.0 / 64.0)));
	private static final float cos27_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 27.0 / 64.0)));
	private static final float cos29_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 29.0 / 64.0)));
	private static final float cos31_64 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 31.0 / 64.0)));
	private static final float cos1_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI / 32.0)));
	private static final float cos3_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 3.0 / 32.0)));
	private static final float cos5_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 5.0 / 32.0)));
	private static final float cos7_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 7.0 / 32.0)));
	private static final float cos9_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 9.0 / 32.0)));
	private static final float cos11_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 11.0 / 32.0)));
	private static final float cos13_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 13.0 / 32.0)));
	private static final float cos15_32 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 15.0 / 32.0)));
	private static final float cos1_16 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI / 16.0)));
	private static final float cos3_16 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 3.0 / 16.0)));
	private static final float cos5_16 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 5.0 / 16.0)));
	private static final float cos7_16 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 7.0 / 16.0)));
	private static final float cos1_8 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI / 8.0)));
	private static final float cos3_8 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI * 3.0 / 8.0)));
	private static final float cos1_4 = (float) (1.0 / (2.0 * Math
			.cos(MY_PI / 4.0)));
	...
	static private float[] load_d() {
		try {
			...
			Object o = JavaLayerUtils.deserializeArrayResource("sfd.ser",
					elemType, 512);
			return (float[]) o;
	...
	/*
	 * static final float d_data[] = { 0.000000000f, -0.000442505f,
	 * 0.003250122f, -0.007003784f, 0.031082153f, -0.078628540f, 0.100311279f,
	 * -0.572036743f, 1.144989014f, 0.572036743f, 0.100311279f, 0.078628540f,
	 * 0.031082153f, 0.007003784f, 0.003250122f, 0.000442505f, -0.000015259f,
	 * -0.000473022f, 0.003326416f, -0.007919312f, 0.030517578f, -0.084182739f,
	 * 0.090927124f, -0.600219727f, 1.144287109f, 0.543823242f, 0.108856201f,
	 * 0.073059082f, 0.031478882f, 0.006118774f, 0.003173828f, 0.000396729f,
	 * -0.000015259f, -0.000534058f, 0.003387451f, -0.008865356f, 0.029785156f,
	 * -0.089706421f, 0.080688477f, -0.628295898f, 1.142211914f, 0.515609741f,
	 * 0.116577148f, 0.067520142f, 0.031738281f, 0.005294800f, 0.003082275f,
	 * 0.000366211f, -0.000015259f, -0.000579834f, 0.003433228f, -0.009841919f,
	 * 0.028884888f, -0.095169067f, 0.069595337f, -0.656219482f, 1.138763428f,
	 * 0.487472534f, 0.123474121f, 0.061996460f, 0.031845093f, 0.004486084f,
	 * 0.002990723f, 0.000320435f, -0.000015259f, -0.000625610f, 0.003463745f,
	 * -0.010848999f, 0.027801514f, -0.100540161f, 0.057617188f, -0.683914185f,
	 * 1.133926392f, 0.459472656f, 0.129577637f, 0.056533813f, 0.031814575f,
	 * 0.003723145f, 0.002899170f, 0.000289917f, -0.000015259f, -0.000686646f,
	 * 0.003479004f, -0.011886597f, 0.026535034f, -0.105819702f, 0.044784546f,
	 * -0.711318970f, 1.127746582f, 0.431655884f, 0.134887695f, 0.051132202f,
	 * 0.031661987f, 0.003005981f, 0.002792358f, 0.000259399f, -0.000015259f,
	 * -0.000747681f, 0.003479004f, -0.012939453f, 0.025085449f, -0.110946655f,
	 * 0.031082153f, -0.738372803f, 1.120223999f, 0.404083252f, 0.139450073f,
	 * 0.045837402f, 0.031387329f, 0.002334595f, 0.002685547f, 0.000244141f,
	 * -0.000030518f, -0.000808716f, 0.003463745f, -0.014022827f, 0.023422241f,
	 * -0.115921021f, 0.016510010f, -0.765029907f, 1.111373901f, 0.376800537f,
	 * 0.143264771f, 0.040634155f, 0.031005859f, 0.001693726f, 0.002578735f,
	 * 0.000213623f, -0.000030518f, -0.000885010f, 0.003417969f, -0.015121460f,
	 * 0.021575928f, -0.120697021f, 0.001068115f, -0.791213989f, 1.101211548f,
